...
- 请使用关键字typename 标识嵌套从属类型名称;但不得在 base class lists（基类列）或member initialization list（成员初值列）内以它作为 baseclass 修饰符。

## 43 学习处理模版化基类内的名称(*)

- 可在派生类模版内通过 this-> 指定 基类模版内的成员名称，或通过一个明白写出 base class 资格修饰符来完成

## 44 将与参数无关的代码抽离templates（*）

- templates生成多个函数和类，因此template代码都不应该与某个造成膨胀的template参数产生相依关系

## 45 运用成员函数模版接受所有兼容类型(*)

## 46 需要类型转换时为模版定义非成员函数（*）
...
- Nothrow new 是一个局限性的工具，只适用于内存分配，后继的构造函数调用还是可能抛出异常

## 50 了解new 和delete的合理替换时机（*）

## 51 编写时需固守常规（*）

## 52 placement new 与 placement delete 成对出现（*）

## 53 不要轻忽编译器的警告

- 严肃对待编译器给出的警告信息，争取做到无警告（或者可以忽略明确知道警告信息意义）
- 不过度依赖编译器报警能力，不同编译器警告级别不同
